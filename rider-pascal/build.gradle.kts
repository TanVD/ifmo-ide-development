...
val resharperPluginPath = File(repoRoot, "Pascal")
val buildConfiguration = ext.properties["BuildConfiguration"] ?: "Debug"

val libFiles = listOf(
        "Pascal/bin/$buildConfiguration/net461/Antlr4.Runtime.Standard.dll"
)

val pluginFiles = listOf(
        "Pascal/bin/$buildConfiguration/net461/JetBrains.ReSharper.Plugins.Pascal")
...
val riderPascalTargetsGroup = "rider-pascal"
...
configure<RdgenParams> {
    val csOutput = File(repoRoot, "Pascal/src/Pascal/src/Protocol")
    val ktOutput = File(repoRoot, "rider-pascal/src/main/java/com/jetbrains/rider/plugins/pascal/protocol")
    ...
    sources(File(repoRoot, "rider-pascal/protocol/src/kotlin/model"))
    packages = "model"
...
""")
    ...
    create("prepare") {
        ...
        doLast {
            exec {
                executable = "dotnet"
                args = listOf("restore", "$resharperPluginPath/Pascal.sln")
            }
        }
    }
    ...
    create("buildReSharperPlugin") {
        ...
        doLast {
            exec {
                executable = "msbuild"
                args = listOf("$resharperPluginPath/Pascal.sln")
            }
        }
    }
